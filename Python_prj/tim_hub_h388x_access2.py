#!/usr/bin/env python3
"""
TIM Hub+ ZTE H388X — accesso HTTP (LAN), variante 2.

FTTH: router connesso in WAN a ONT (es. FiberHome FG1000R), profilo INTERNET_ETH.

Rispetto a tim_hub_h388x_access.py:
  - JSON con tutti i parametri INTERNET_ETH esposti dal modem
  - WAN_STATUS include tempo_connessione, connessione_attiva, velocita
  - WAN_STATUS.stato_connessione_ipv4_con_ip_valido = OK | KO
    (stato tipo "Stato connessione IPv4" = Connesso + indirizzo IPv4 valido, non 0.0.0.0 ecc.)

Uso:
  python tim_hub_h388x_access2.py --host 192.168.1.1 --password "..." internet-eth
  python tim_hub_h388x_access2.py --host 192.168.1.1 --password-from-env TIM_HUB_PASSWORD wan-status
...
def _norm_key_compact(k: str) -> str:
    return re.sub(r"[^a-z0-9]", "", k.lower())


def _is_ipv4_connesso_value(raw: str) -> bool:
    """True se il valore indica IPv4 connesso (es. Connesso / Connected)."""
    s = raw.strip().lower()
    if not s:
        return False
    if "disconn" in s or s in {"down", "offline", "no", "inactive", "idle", "0", "false"}:
        return False
    if "connesso" in s or "connected" in s or s in {"up", "online", "1", "true"}:
        return True
    if s.startswith("establish") and "fail" not in s:
        return True
    return False


def _is_valid_public_style_ipv4(s: str) -> bool:
    """
    IPv4 assegnabile/useful per WAN: esclude 0.0.0.0, loopback, link-local APIPA,
    multicast, non specificato, non valido sintatticamente.
    """
    t = s.strip()
    if not t or t == "0.0.0.0":
        return False
    try:
...
def _find_ipv4_connection_status(row: dict[str, str]) -> tuple[Optional[str], Optional[str]]:
    """Cerca il parametro stato connessione IPv4 (GUI: Stato connessione IPv4 / Connesso)."""
    best: Optional[tuple[int, str, str]] = None
    for k, v in row.items():
        if k.startswith("_"):
            continue
        nk = _norm_key_compact(k)
        if "ipv6" in nk:
            continue
        if "ipv4" not in nk:
            continue
        if ("address" in nk or "addr" in nk) and "conn" not in nk and "status" not in nk:
            continue
        if not any(x in nk for x in ("conn", "status", "state", "oper", "stato")):
            continue
        score = 10 + (5 if "conn" in nk else 0) + (3 if "wan" in nk else 0)
    ...
    for k, v in row.items():
        ...
        if "ipv6" in nk:
            continue
        if nk == "connstatus" or nk.endswith("connstatus") or "connectionstatus" in nk:
            return k, v
    return None, None


def _find_wan_ipv4_address(row: dict[str, str]) -> tuple[Optional[str], Optional[str]]:
    """Indirizzo IPv4 WAN (non gateway/DNS/mask se evitabili)."""
    ...
    for k, v in row.items():
        ...
        if "ipv6" in nk:
            continue
        if "dns" in nk or "gateway" in nk or "gw" == nk[-2:]:
            if "wan" not in nk and "external" not in nk:
                continue
        t = (v or "").strip()
        if not t or not re.match(r"^\d{1,3}(\.\d{1,3}){3}$", t):
            continue
        score = 0
        if "ipv4" in nk:
            score += 10
        if any(x in nk for x in ("external", "wan", "public", "ppp", "nat")):
            score += 15
        if "address" in nk or "addr" in nk or nk.endswith("ip"):
            score += 5
        if "subnet" in nk or "mask" in nk or "prefix" in nk:
            score -= 20
        if score > -10:
    ...
    for k, v in row.items():
        if k.startswith("_"):
            continue
        nk = _norm_key_compact(k)
        if "ipv6" in nk or "mask" in nk or "dns" in nk:
            continue
        if nk in ("externalipaddress", "ipaddress", "wanipaddress"):
            t = (v or "").strip()
            if t and re.match(r"^\d{1,3}(\.\d{1,3}){3}$", t):
                return k, v
    return None, None


def _assess_ipv4_connesso_e_ip(row: dict[str, str]) -> dict[str, Any]:
    sk, sv = _find_ipv4_connection_status(row)
    ik, iv = _find_wan_ipv4_address(row)
    conn_ok = bool(sv and _is_ipv4_connesso_value(sv))
    ip_ok = bool(iv and _is_valid_public_style_ipv4(iv))
    esito = "OK" if conn_ok and ip_ok else "KO"
    motivi: list[str] = []
    if not sk:
        motivi.append("parametro_stato_connessione_ipv4_non_individuato")
    elif not conn_ok:
        motivi.append("stato_connessione_ipv4_non_connesso")
    if not ik:
        motivi.append("parametro_indirizzo_ipv4_wan_non_individuato")
    elif not ip_ok:
        motivi.append("indirizzo_ipv4_assente_non_valido_o_non_usabile")
    return {
        "esito": esito,
        "stato_connessione_ipv4_chiave_modem": sk,
        "stato_connessione_ipv4_valore": sv,
        "indirizzo_ipv4_chiave_modem": ik,
        "indirizzo_ipv4_valore": iv,
        "motivi_ko": motivi if esito == "KO" else None,
    }
...
def build_wan_status(
    ...
    ipv4_eval = _assess_ipv4_connesso_e_ip(main)
    ...
    return {
        ...
        "stato_connessione_ipv4_con_ip_valido": ipv4_eval["esito"],
        "dettaglio_stato_e_ip_ipv4": {
            "stato_connessione_ipv4_chiave_modem": ipv4_eval["stato_connessione_ipv4_chiave_modem"],
            "stato_connessione_ipv4_valore": ipv4_eval["stato_connessione_ipv4_valore"],
            "indirizzo_ipv4_chiave_modem": ipv4_eval["indirizzo_ipv4_chiave_modem"],
            "indirizzo_ipv4_valore": ipv4_eval["indirizzo_ipv4_valore"],
            "motivi_ko": ipv4_eval["motivi_ko"],
        },
    }
...
def cmd_wan_status(client: TimHubH388XClient, linetype: str) -> int:
    ...
    try:
        ...
        primary = internet_eth_rows[0] if internet_eth_rows else {}
        summary: dict[str, Any] = {
            "script": "tim_hub_h388x_access2",
            "line_type": linetype,
            "INTERNET_ETH_tutti_i_parametri": primary,
            "INTERNET_ETH_istanze": internet_eth_rows,
            "eth_or_dsl_interface": eth_iface,
            "wan_internet_tutte_le_istanze": wan_rows,
            "WAN_STATUS": build_wan_status(internet_eth_rows, eth_iface),
            "nota": (
                "INTERNET_ETH_tutti_i_parametri = mappa chiave/valore come restituita dal modem per la prima "
                "istanza riconosciuta; INTERNET_ETH_istanze elenca tutte le righe filtrate."
            ),
        }
    ...
    try:
        ...
        if not internet_eth_rows:
            out["warning"] = (
                ...
                "Usa wan-status o dump-raw: il _InstID sul firmware potrebbe differire."
            )
...
def main() -> int:
    p = argparse.ArgumentParser(description="TIM Hub+ ZTE H388X — access2 (INTERNET_ETH completo + WAN_STATUS)")
    ...
    p.add_argument("--password-from-env", metavar="VAR", help="Password da variabile d'ambiente")
    p.add_argument(
        "--linetype",
        choices=("eth", "dsl"),
        default=os.environ.get("TIM_HUB_LINETYPE", "eth"),
        help="eth = FTTH / ONT; dsl = xDSL",
    )
    p.add_argument(
        "--https",
        action="store_true",
    ...
    p.add_argument(
        ...
        help="internet-eth | wan-status (include tutti i campi INTERNET_ETH) | dump-raw",
    )
    args = p.parse_args()
    ...
    client = TimHubH388XClient(args.host, args.user, password, use_https=args.https)
